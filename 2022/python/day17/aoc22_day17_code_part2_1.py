...
class Space:
    ...
    def _drop_shape(self, shape, x, y, pushdir):
        at_rest = False

        if not self.check_shape_fits(shape, x, y):
            # print("Shape cannot be dropped here!")
            return

        (new_x, new_y) = (x + pushdir, y)
    ...
    def drop_shape(self, rock, shapeid, start_x, start_y):
        ...
        while not at_rest:
            pushdir = AIRJET_MOVE_X[airjets[self.airjet]]
            (xxx) = self._drop_shape(shape, x, y, pushdir)
            self.airjet += 1
...
airjets = data
rock = 0
rocks_to_drop = 20  # 1000000000000
while rock < rocks_to_drop:
    space.drop_shape(rock, rock % num_shapes, shape_start_x, space.get_shape_starting_y())
    rock += 1
